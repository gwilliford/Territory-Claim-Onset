...
# Leader support data
ead <- read.dta13("./data/EAD+2.0+Annual+0101019.dta")
ead$ccode = as.numeric(ead$ccode)
...
terrchange$one = 1
terrchange = terrchange %>% group_by(dyad, year) %>% summarize(
  terrchangedum = 1,
  terrchangenum = sum(one)
)
...
ddy <- left_join(ddy, tek)
ddy <- full_join(ddy, terrchange)
...
dat$caprat <- rowMins(cbind(dat$cinc1, dat$cinc2)) / (dat$cinc1 + dat$cinc2)
...
dat$terrchangedum = ifelse(is.na(dat$terrchangedum), 0, 1)
dat = dat %>% 
  arrange(dyad, year) %>% 
  group_by(dyad) %>% 
  mutate(
    terrchangenum = cumsum(terrchangenum),
